    ...
    *song_length = length;
    ...
    do
    {
        ...
        if (first_time && !frameInfo.error)
        {
			...
			dlg->AppendBInfo("FrameInfo","Channels",tempstr,"Channels");
			memset(tempstr,0,100);
			//采样率
			itoa(frameInfo.samplerate,tempstr,10);
			dlg->AppendBInfo("FrameInfo","Sample Rate",tempstr,"Sample Rate");
			memset(tempstr,0,100);
			//AAC类型-------
			tempnum=frameInfo.object_type;
			switch (tempnum){
			case MAIN:strcpy(tempstr,"MAIN");break;
			case LC:strcpy(tempstr,"LC");break;
			case SSR:strcpy(tempstr,"SSR");break;
			case LTP:strcpy(tempstr,"LTP");break;
			case HE_AAC:strcpy(tempstr,"HE_AAC");break;
			case ER_LC:strcpy(tempstr,"ER_LC");break;
			case ER_LTP:strcpy(tempstr,"ER_LTP");break;
			case LD:strcpy(tempstr,"LD");break;
			case DRM_ER_LC:strcpy(tempstr,"DRM_ER_LC");break;
			}
			dlg->AppendBInfo("FrameInfo","Oject Type",tempstr,"Oject Type");
			memset(tempstr,0,100);
			//Header Type
			tempnum=frameInfo.header_type;
			switch (tempnum){
			case RAW:strcpy(tempstr,"RAW");break;
			case ADIF:strcpy(tempstr,"ADIF");break;
			case ADTS:strcpy(tempstr,"ADTS");break;
			case LATM:strcpy(tempstr,"LATM");break;
			}
			dlg->AppendBInfo("FrameInfo","Header Type",tempstr,"Header Type");
			memset(tempstr,0,100);

			//SBR
			tempnum=frameInfo.sbr;
			switch (tempnum){
			case NO_SBR:strcpy(tempstr,"off");break;
			case SBR_UPSAMPLED:strcpy(tempstr,"on;upsample");break;
			case SBR_DOWNSAMPLED:strcpy(tempstr,"on;downsampled");break;
			case NO_SBR_UPSAMPLED:strcpy(tempstr,"off;upsampled");break;
			}
			dlg->AppendBInfo("FrameInfo","SBR",tempstr,"HE-AAC(AAC+SBR)");
			memset(tempstr,0,100);
			//PS
			tempnum=frameInfo.ps;
			switch (tempnum){
			case 0:strcpy(tempstr,"off");break;
			case 1:strcpy(tempstr,"on");break;
			}
			dlg->AppendBInfo("FrameInfo","PS",tempstr,"HE-AAC v2(AAC+SBR+PS)");
			free(tempstr);
			//-------------
            if (!adts_out)
            {
...
#define PRINT_MP4_METADATA
	...
	dlg->AppendBInfo("Container","objectTypeIndex",tempstr,"object Type Index");
	//samplingFrequencyIndex
	tempnum=mp4ASC.samplingFrequencyIndex;
	itoa(tempnum,tempstr,10);
	dlg->AppendBInfo("Container","samplingFrequencyIndex",tempstr,"sampling Frequency Index");
	//samplingFrequency
	itoa(mp4ASC.samplingFrequency,tempstr,10);
	dlg->AppendBInfo("Container","samplingFrequency",tempstr,"sampling Frequency");
	memset(tempstr,0,100);
	//channelsConfiguration
	tempnum=mp4ASC.channelsConfiguration;
	itoa(tempnum,tempstr,10);
	dlg->AppendBInfo("Container","channelsConfiguration",tempstr,"channels Configuration");
	free(tempstr);
	//-----------
	//输出MetaData
	//此处使用枚举的方法
	...
	for (y = 0; y < x; y++)
	{
		if (mp4ff_meta_get_by_index(infile, y, &item, &tag))
		{
			if (item != NULL && tag != NULL)
			{
				dlg->AppendBInfo("MetaData",item,tag,tag);
				free(item); item = NULL;
				free(tag); tag = NULL;
			}
		}
...
/* System dependant types */
    ...
    if (mp4file)
    {
		dlg->AppendBInfo("Container","Container","MP4","MP4 Container Format");
		...
		dlg->AppendBInfo("Container","Container","RAW","RAW Format");
